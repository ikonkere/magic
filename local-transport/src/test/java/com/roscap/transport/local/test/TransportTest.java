package com.roscap.transport.local.test;
...
import com.roscap.mw.transport.local.InMemoryTransportAdapter;

@RunWith(BlockJUnit4ClassRunner.class)
public class TransportTest {
	InMemoryTransportAdapter transportAdapter;
...
//	Sender s;
	...
	@Before
	public void init() {
		transportAdapter = new InMemoryTransportAdapter();
	...
	private class Receiver implements TransportEndpoint<Dummy<UUID>> {
		...
		public void receive(String fromDestination, Dummy<UUID> payload) {
			...
			transportAdapter.send(payload.getHeader(TransportHeader.CORRELATION).toString(), payload);
		}
